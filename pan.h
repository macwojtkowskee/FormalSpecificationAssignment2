...
#define PanSource	"trash_bin.pml"
...
#define _nstates4	56	/* main_control */
#define minseq4	143
#define maxseq4	197
#define _endstate4	55
...
struct bin_t { /* user defined type */
	uchar out_door;
	uchar lock_out_door;
	uchar trap_door;
	uchar ram;
	uchar trash_in_outer_door;
	uchar trash_on_trap_door;
	uchar trash_compressed;
	uchar trash_uncompressed;
	unsigned full_capacity : 1;
	unsigned trap_destroyed : 1;
};
...
#define Air3	(sizeof(P3) - Offsetof(P3, bin_id) - 1*sizeof(uchar))

#define Ptruck	((P2 *)_this)
typedef struct P2 { /* truck */
	unsigned _pid : 8;  /* 0..255 */
	unsigned _t   : 4; /* proctype */
	unsigned _p   : 8; /* state    */
#ifdef HAS_PRIORITY
...
#define Air2	(sizeof(P2) - Offsetof(P2, bin_id) - 1*sizeof(uchar))
...
#define Air1	(sizeof(P1) - Offsetof(P1, user_id) - 1*sizeof(uchar))

#define Pbin	((P0 *)_this)
typedef struct P0 { /* bin */
	unsigned _pid : 8;  /* 0..255 */
	unsigned _t   : 4; /* proctype */
	unsigned _p   : 8; /* state    */
#ifdef HAS_PRIORITY
...
#define Air0	(sizeof(P0) - Offsetof(P0, bin_id) - 1*sizeof(uchar))
...
typedef struct State {
	...
	uchar weigh_trash;
	uchar trash_weighted;
	uchar change_ram;
	uchar ram_changed;
	uchar scan_card_user;
	uchar can_deposit_trash;
	uchar check_user;
	uchar user_valid;
	uchar request_truck;
	uchar change_truck;
